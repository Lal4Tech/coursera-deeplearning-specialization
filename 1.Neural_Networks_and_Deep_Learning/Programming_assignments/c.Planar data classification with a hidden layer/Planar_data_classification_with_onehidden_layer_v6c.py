...
def compute_cost(A2, Y, parameters):
    ...
    #logprobs = np.dot(Y.T, np.log(A2)) + np.dot(np.ones(Y.T.shape) - Y.T, np.log(np.ones(A2.shape) - A2))
    #cost = -logprobs
...
def backward_propagation(parameters, cache, X, Y):
    ...
    dZ2 = A2 - Y
    dW2 = (1 / m) * np.dot(dZ2, A1.T) # (1, m) x (m, nh) => (1, nh)
    db2 = (1 / m) * np.sum(dZ2, axis=1, keepdims=True) # (1, 1)
    dZ1 = np.multiply(np.dot(W2.T, dZ2), (1 - np.power(A1, 2))) # (nh, m)
    #dZ1 = np.multiply(np.dot(W2.T, dZ2), (A1 > 0)) # (nh, m) for ReLU
    dW1 = (1 / m) * np.dot(dZ1, X.T) # (nh, nx)
